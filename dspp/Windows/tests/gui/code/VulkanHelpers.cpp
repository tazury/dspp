...
bool VulkanHelpers::UpdateUIScaling(float scale)
{
    ...
    // ANTON FONT
    if (resources.LoadData(IDR_FONT1, &fontData, &fontSize)) {
        ImGuiIO& io = ImGui::GetIO();
        ImFontConfig font_cfg;
        font_cfg.FontDataOwnedByAtlas = false;  // Prevent ImGui from freeing memory

        io.Fonts->AddFontFromMemoryTTF(fontData, fontSize, 16.0f * scale, &font_cfg);
    }
    else {
        OPLogger::get().Error("This executable of DS++ is not proper, please redownload it.", 1);
    }
